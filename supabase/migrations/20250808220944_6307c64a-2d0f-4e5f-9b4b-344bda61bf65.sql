-- Fix invalid existing data and add constraints/indexes safely

-- 1) Normalize data to satisfy constraints
-- Set non-positive standard_position to NULL
UPDATE public.part_number_codes
SET standard_position = NULL
WHERE standard_position IS NOT NULL AND standard_position <= 0;

-- Remove non-positive or NULL entries from designated_positions
UPDATE public.part_number_codes
...
-- 2) Add constraints as NOT VALID to avoid blocking on any remaining edge cases
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    ...
    CHECK (
      ...
      OR NOT EXISTS (
        SELECT 1 FROM UNNEST(designated_positions) AS v
        WHERE v IS NULL OR v <= 0
      )
    ) NOT VALID;
  END IF;
...
-- 4) Clarifying comments (idempotent)
