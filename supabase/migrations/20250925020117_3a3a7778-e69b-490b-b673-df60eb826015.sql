-- Update generate_quote_id function to include user email prefix
CREATE OR REPLACE FUNCTION public.generate_quote_id(user_email text DEFAULT NULL, is_draft boolean DEFAULT false)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    -- Generate the new ID with format: email_prefix-quote_prefix-counter
    IF is_draft THEN
        quote_id := email_prefix || '-' || prefix_setting || '-' || counter_setting || '-Draft';
    ELSE
        quote_id := email_prefix || '-' || prefix_setting || '-' || counter_setting;
    END IF;
