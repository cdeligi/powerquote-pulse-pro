-- PowerQuotePro: Complete Database Schema Migration for Real Product Data
-- This migration creates the proper schema required for the user's specifications

-- 1. Drop old inconsistent tables and start fresh
DROP TABLE IF EXISTS products_lvl1, products_lvl2, products_lvl3 CASCADE;

-- 2. Enhance the main products table to handle all levels 1-3
ALTER TABLE products DROP CONSTRAINT IF EXISTS products_pkey;
DROP INDEX IF EXISTS products_pkey;
ALTER TABLE products ALTER COLUMN id TYPE text;
ALTER TABLE products ADD PRIMARY KEY (id);

-- Add missing required columns for proper product management
...
-- Update existing data to have proper product levels
UPDATE products SET product_level = 1 WHERE category = 'level1';
UPDATE products SET product_level = 2 WHERE category = 'level2';  
UPDATE products SET product_level = 3 WHERE category = 'level3';

-- 3. Update level4_products table to match interface requirements
ALTER TABLE level4_products 
ADD COLUMN IF NOT EXISTS code text,
ADD COLUMN IF NOT EXISTS part_number text;
...
CREATE INDEX IF NOT EXISTS idx_level4_parent ON level4_products(parent_product_id);

-- 5. Insert default Level 1 products with rack_configurable settings
...
ON CONFLICT (id) DO UPDATE SET
  ...
  rack_configurable = EXCLUDED.rack_configurable,
  product_level = EXCLUDED.product_level,
  part_number = EXCLUDED.part_number,
  image_url = EXCLUDED.image_url,
  product_info_url = EXCLUDED.product_info_url;

-- 6. Insert Level 2 products (chassis for QTMS)
...
-- 7. Insert Level 3 products (cards)
...
ON CONFLICT (id) DO UPDATE SET
  ...
  product_level = EXCLUDED.product_level,
  part_number = EXCLUDED.part_number,
  slot_requirement = EXCLUDED.slot_requirement,
  specifications = EXCLUDED.specifications;

-- 8. Duplicate cards for MTX and STX chassis
...
-- 9. Insert sample Level 4 configurations for analog cards
INSERT INTO level4_products (id, name, parent_product_id, description, configuration_type, price, cost, enabled) VALUES
('analog-sensor-config', 'Analog Sensor Configuration', 'analog-card-multi', 'Configure analog sensor types and parameters', 'dropdown', 0, 0, true),
('bushing-tap-config', 'Bushing Tap Configuration', 'bushing-card', 'Configure bushing tap models and settings', 'multiline', 0, 0, true)
ON CONFLICT (id) DO UPDATE SET
  name = EXCLUDED.name,
  parent_product_id = EXCLUDED.parent_product_id,
  description = EXCLUDED.description,
...
-- 10. Insert Level 4 configuration options
INSERT INTO level4_configuration_options (level4_product_id, option_key, option_value, display_order, enabled) VALUES
('analog-sensor-config', 'temperature', 'Temperature Sensor', 1, true),
('analog-sensor-config', 'pressure', 'Pressure Sensor', 2, true),
('analog-sensor-config', 'current', 'Current Sensor', 3, true),
('analog-sensor-config', 'voltage', 'Voltage Sensor', 4, true),
('bushing-tap-config', 'tap_model', 'Bushing Tap Model Selection', 1, true),
('bushing-tap-config', 'measurement_type', 'Measurement Type Configuration', 2, true),
('bushing-tap-config', 'alarm_settings', 'Alarm and Warning Settings', 3, true)
ON CONFLICT DO NOTHING;

-- 11. Clean up old relationship tables (we now use parent_product_id directly)
UPDATE level1_level2_relationships SET level2_product_id = level2_product_id WHERE 1=1;
UPDATE level2_level3_relationships SET level3_product_id = level3_product_id WHERE 1=1;

-- 12. Update RLS policies to work with new schema
...
-- 13. Enable RLS on level4 tables if not already enabled
ALTER TABLE level4_products ENABLE ROW LEVEL SECURITY;
ALTER TABLE level4_configuration_options ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "Users can view level4 products" ON level4_products
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Admins can manage level4 products" ON level4_products
  FOR ALL USING (get_user_role(auth.uid()) = 'admin');

CREATE POLICY IF NOT EXISTS "Users can view level4 options" ON level4_configuration_options
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Admins can manage level4 options" ON level4_configuration_options  
  FOR ALL USING (get_user_role(auth.uid()) = 'admin');