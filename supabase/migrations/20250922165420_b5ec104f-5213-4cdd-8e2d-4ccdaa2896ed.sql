...
-- Update quote access policies to include shared quotes
CREATE OR REPLACE POLICY "Users can view their own quotes and shared quotes" 
ON public.quotes 
FOR SELECT 
USING (
  user_id = auth.uid() OR 
...
CREATE OR REPLACE POLICY "Users can update their own quotes and shared quotes with edit permission" 
ON public.quotes 
FOR UPDATE 
USING (
  user_id = auth.uid() OR 
